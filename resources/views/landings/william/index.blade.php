...
<head>
	...
	<style>
		...
		* { margin: 0; padding: 0; box-sizing: border-box; }
		...
		.navbar-brand img {
			max-width: 180px; height: auto; max-height: 60px; object-fit: contain; transition: transform 0.3s ease;
		}
		.navbar-brand:hover img { transform: scale(1.05); }
		.nav-link {
			color: var(--dark) !important; font-weight: 500; margin: 0 15px; position: relative; transition: color 0.3s ease; font-size: 0.95rem;
		}
		.nav-link::after {
			content: '';
			position: absolute; bottom: -5px; left: 50%; width: 0; height: 2px; background: var(--primary-color);
		...
		.btn-cta-header {
			background: var(--primary-color); color: white; padding: 10px 25px; border-radius: 50px; font-weight: 600; transition: all 0.3s ease; border: 2px solid var(--primary-color); font-size: 0.9rem;
		}
		.btn-cta-header:hover {
			background: transparent; color: var(--primary-color); transform: translateY(-2px); box-shadow: 0 5px 15px rgba(42, 102, 255, 0.3);
		}

		/* Hero */
		.hero-section { position: relative; min-height: 100vh; display: flex; align-items: center; background: linear-gradient(135deg, var(--soft-white) 0%, var(--soft-gray) 100%); overflow: hidden; padding-top: 100px; }
		.hero-section::before { content: ''; position: absolute; top: 0; right: -50%; width: 100%; height: 100%; background: linear-gradient(45deg, transparent 0%, rgba(42, 102, 255, 0.03) 50%, transparent 100%); transform: rotate(-15deg); }
		.hero-content { position: relative; z-index: 2; }
		.hero-title { font-size: 3.2rem; font-weight: 700; color: var(--dark); margin-bottom: 20px; line-height: 1.1; }
		.hero-subtitle { font-size: 1.4rem; color: var(--primary-color); margin-bottom: 20px; font-weight: 600; }
		.hero-description { font-size: 1.1rem; color: #666; margin-bottom: 40px; line-height: 1.8; }
		.hero-image { position: relative; animation: float 6s ease-in-out infinite; }
		.hero-image img { max-width: 100%; height: auto; border-radius: 20px; box-shadow: 0 20px 60px rgba(0,0,0,0.1); }
		@keyframes float { 0%, 100% { transform: translateY(0px); } 50% { transform: translateY(-20px); } }

		/* Buttons */
		.btn-primary-custom { background: var(--primary-color); color: white; padding: 14px 34px; border-radius: 50px; font-weight: 600; font-size: 1.05rem; border: 2px solid var(--primary-color); transition: all 0.3s ease; text-decoration: none; display: inline-block; margin: 10px; }
		.btn-primary-custom:hover { background: transparent; color: var(--primary-color); transform: translateY(-3px); box-shadow: 0 10px 30px rgba(42, 102, 255, 0.3); }
		.btn-outline-custom { background: transparent; color: var(--dark); padding: 14px 34px; border-radius: 50px; font-weight: 600; font-size: 1.05rem; border: 2px solid var(--dark); transition: all 0.3s ease; text-decoration: none; display: inline-block; margin: 10px; }
		.btn-outline-custom:hover { background: var(--dark); color: white; transform: translateY(-3px); box-shadow: 0 10px 30px rgba(29, 29, 31, 0.3); }

		/* Sections */
		.section { padding: 90px 0; position: relative; }
		.section-title { font-size: 2.4rem; font-weight: 700; color: var(--dark); margin-bottom: 20px; text-align: center; }
		.section-subtitle { font-size: 1.1rem; color: #666; text-align: center; margin-bottom: 55px; max-width: 680px; margin-left: auto; margin-right: auto; }

		/* About */
		.about-section { background: white; }
		.about-card { background: white; padding: 32px; border-radius: 20px; box-shadow: 0 10px 40px rgba(0,0,0,0.08); transition: transform 0.3s ease; border: 1px solid rgba(0,0,0,0.05); }
		.about-card:hover { transform: translateY(-5px); }
		.about-icon { width: 72px; height: 72px; background: linear-gradient(135deg, var(--primary-color), #4285f4); border-radius: 18px; display: flex; align-items: center; justify-content: center; margin-bottom: 24px; color: white; font-size: 1.8rem; }

		/* Problems */
		.problems-section { background: var(--soft-gray); }
		.problem-card { background: white; padding: 32px; border-radius: 20px; text-align: center; margin-bottom: 24px; box-shadow: 0 10px 40px rgba(0,0,0,0.08); transition: all 0.3s ease; border: 1px solid rgba(0,0,0,0.05); height: 100%; }
		.problem-card:hover { transform: translateY(-5px); box-shadow: 0 20px 60px rgba(0,0,0,0.15); }
		.problem-icon { width: 72px; height: 72px; background: linear-gradient(135deg, #ff6b6b, #ff5252); border-radius: 50%; display: flex; align-items: center; justify-content: center; margin: 0 auto 20px; color: white; font-size: 1.6rem; }

		/* Benefits */
		.benefits-section { background: white; }
		.benefit-card { background: linear-gradient(135deg, var(--primary-color), #4285f4); color: white; padding: 32px; border-radius: 20px; text-align: center; margin-bottom: 24px; transition: all 0.3s ease; border: none; height: 100%; }
		.benefit-card:hover { transform: translateY(-5px) scale(1.02); box-shadow: 0 20px 60px rgba(42, 102, 255, 0.3); }
		...
		/* Gallery */
		.gallery-section { background: var(--soft-gray); }
		.gallery-item { border-radius: 18px; overflow: hidden; margin-bottom: 24px; box-shadow: 0 15px 50px rgba(0,0,0,0.1); transition: all 0.3s ease; background: white; position: relative; cursor: pointer; }
		.gallery-item:hover { transform: translateY(-8px); box-shadow: 0 25px 70px rgba(0,0,0,0.2); }
		.gallery-item img { width: 100%; height: 300px; object-fit: cover; transition: transform 0.3s ease; max-width: 100%; }
		.gallery-item:hover img { transform: scale(1.05); }
		.gallery-overlay { position: absolute; inset: 0; background: rgba(42, 102, 255, 0.9); color: white; display: flex; flex-direction: column; align-items: center; justify-content: center; opacity: 0; transition: all 0.3s ease; backdrop-filter: blur(5px); }
		.gallery-item:hover .gallery-overlay { opacity: 1; }
		.gallery-overlay i { font-size: 2.4rem; margin-bottom: 10px; animation: pulse 2s infinite; }
		.gallery-overlay p { font-size: 1rem; font-weight: 600; margin: 0; text-align: center; padding: 0 16px; }

		/* Gallery Modal */
		.gallery-modal .modal-dialog { max-width: 90vw; margin: 30px auto; }
		.gallery-modal .modal-dialog .modal-content { overflow: hidden; }
		.gallery-modal .modal-content { background: transparent; border: none; box-shadow: none; }
		.gallery-modal .modal-header { background: rgba(0,0,0,0.8); border: none; border-radius: 0; padding: 15px 20px; }
		.gallery-modal .modal-body { padding: 0; position: relative; background: rgba(0,0,0,0.9); border-radius: 0 0 10px 10px; overflow: hidden; display: flex; align-items: center; justify-content: center; min-height: 300px; }
		.gallery-modal .modal-image { width: 100%; max-width: 100%; max-height: 70vh; object-fit: contain; display: block; margin: 0 auto; }
		.gallery-nav-btn { position: absolute; top: 50%; transform: translateY(-50%); background: rgba(42, 102, 255, 0.8); color: white; border: none; border-radius: 50%; width: 50px; height: 50px; font-size: 1.1rem; cursor: pointer; transition: all 0.3s ease; z-index: 1001; }
		.gallery-nav-btn:hover { background: var(--primary-color); transform: translateY(-50%) scale(1.1); }
		.gallery-nav-btn.prev { left: 20px; }
		.gallery-nav-btn.next { right: 20px; }
		.gallery-counter { position: absolute; bottom: 16px; left: 50%; transform: translateX(-50%); background: rgba(0,0,0,0.7); color: white; padding: 6px 12px; border-radius: 16px; font-size: 0.9rem; z-index: 1001; }

		/* Community */
		.community-section { background: white; }
		.testimonial-card { background: white; padding: 32px; border-radius: 20px; box-shadow: 0 10px 40px rgba(0,0,0,0.08); text-align: center; margin-bottom: 24px; border: 1px solid rgba(0,0,0,0.05); transition: transform 0.3s ease; height: 100%; }
		.testimonial-card:hover { transform: translateY(-5px); }
		.testimonial-avatar { width: 72px; height: 72px; border-radius: 50%; margin: 0 auto 16px; background: linear-gradient(135deg, var(--primary-color), #4285f4); display: flex; align-items: center; justify-content: center; color: white; font-size: 1.6rem; font-weight: bold; }
		.stars { color: #ffc107; font-size: 1.1rem; margin-bottom: 12px; }

		/* Contact */
		.contact-section { background: var(--dark); color: white; }
		.contact-card { background: rgba(255,255,255,0.1); padding: 32px; border-radius: 18px; text-align: center; margin-bottom: 24px; backdrop-filter: blur(10px); border: 1px solid rgba(255,255,255,0.1); transition: transform 0.3s ease; height: 100%; }
		.contact-card:hover { transform: translateY(-5px); background: rgba(255,255,255,0.15); }
		.contact-icon { width: 72px; height: 72px; background: var(--primary-color); border-radius: 50%; display: flex; align-items: center; justify-content: center; margin: 0 auto 18px; font-size: 1.6rem; }

		/* Footer */
		.footer { background: #000; color: white; padding: 60px 0 30px; }
		.footer-logo img { max-height: 60px; margin-bottom: 20px; }
		.social-links a { display: inline-block; width: 44px; height: 44px; background: rgba(255,255,255,0.1); border-radius: 50%; text-align: center; line-height: 44px; color: white; margin: 0 8px 8px 0; transition: all 0.3s ease; font-size: 1.1rem; }
		...
		.modal-header { background: var(--primary-color); color: white; border-radius: 20px 20px 0 0; }
		.modal-body { padding: 28px; }

		/* WhatsApp Floating */
		.whatsapp-float { position: fixed; width: 60px; height: 60px; bottom: 40px; right: 40px; background-color: #25d366; color: #FFF; border-radius: 50px; text-align: center; font-size: 30px; box-shadow: 2px 2px 3px #999; z-index: 1000; animation: pulse 2s infinite; display: flex; align-items: center; justify-content: center; text-decoration: none; }
		.whatsapp-float:hover { color: #FFF; background-color: #20ba5a; }
		...
		@media (max-width: 768px) {
			...
			.navbar-brand img { max-width: 140px; height: auto; max-height: 45px; object-fit: contain; }
			.gallery-modal .modal-dialog { max-width: 95vw; margin: 15px auto; }
...
<body>
	@php
		// Contact fallbacks
		$whatsNumber = $empresa->whatsapp ?: $empresa->movil;
		$hasWhats = !empty($whatsNumber);
		$hasEmail = !empty($empresa->email);
		$hasPhone = !empty($empresa->movil);
		$tiendaActiva = isset($productosActivos) && $productosActivos > 0;
	@endphp
					...
					<li class="nav-item">
						...
						@elseif($hasEmail)
							<a class="btn btn-cta-header" href="mailto:{{ $empresa->email }}?subject=Quiero%20información%20sobre%20{{ urlencode($landing->titulo_principal) }}">
								<i class="fas fa-envelope me-2"></i>Escríbenos
							</a>
						@endif
					</li>
				</ul>
				...
				<div class="col-lg-6" data-aos="fade-left">
					<div class="hero-image">
						@if($landing->media && $landing->media->count() > 0)
							<img src="{{ asset('storage/' . $landing->media->first()->url) }}" alt="{{ $landing->media->first()->descripcion ?? ('Imagen de ' . $empresa->nombre) }}" class="animate__animated animate__fadeIn animate__delay-1s">
						@else
				...
				@if($landing->media && $landing->media->count() > 0)
					@foreach($landing->media as $index => $media)
						<div class="col-lg-6 col-md-6 mb-4" data-aos="fade-up" data-aos-delay="{{ $index * 100 }}">
							<div class="gallery-item" data-bs-toggle="modal" data-bs-target="#galleryModal" data-image="{{ asset('storage/' . $media->url) }}" data-description="{{ $media->descripcion ?? ('Imagen de ' . $empresa->nombre) }}" data-index="{{ $index }}">
				...
				@else
					@for($i = 0; $i < 4; $i++)
						<div class="col-lg-6 col-md-6 mb-4" data-aos="fade-up" data-aos-delay="{{ $i * 100 }}">
							<div class="gallery-item" data-bs-toggle="modal" data-bs-target="#galleryModal" data-image="{{ asset('storage/' . $landing->logo_url) }}" data-description="Imagen {{ $i + 1 }} - {{ $empresa->nombre }}" data-index="{{ $i }}">
								<img src="{{ asset('storage/' . $landing->logo_url) }}" alt="Imagen {{ $i + 1 }} - {{ $empresa->nombre }}">
								<div class="gallery-overlay">
									<i class="fas fa-search-plus"></i>
									<p>Ver imagen</p>
								</div>
				...
				@foreach($testimonials as $index => $testimonial)
					<div class="col-lg-4 col-md-6 mb-4" data-aos="fade-up" data-aos-delay="{{ $index * 100 }}">
						<div class="testimonial-card h-100">
							...
							<p>"{!! str_replace(['{{ $empresa->nombre }}'], [$empresa->nombre], $testimonial['comment']) !!}"</p>
	...
	<section id="contact" class="section contact-section">
		<div class="container">
			...
			<p class="section-subtitle" data-aos="fade-up" data-aos-delay="100">Estamos aquí para ayudarte</p>
				...
				@if($hasPhone)
					<div class="col-lg-3 col-md-6 mb-4" data-aos="fade-up" data-aos-delay="300">
						<div class="contact-card h-100">
							...
							<h5>Teléfono</h5>
							<p>{{ $empresa->movil }}</p>
							<a href="tel:+57{{ preg_replace('/[^0-9]/', '', $empresa->movil) }}" class="btn btn-outline-light btn-sm">Llamar</a>
						</div>
					</div>
				@endif
				...
				<div class="col-lg-4 mb-4">
					...
					<div class="social-links">
						...
						@if(!empty($whatsNumber))
							<a href="https://wa.me/57{{ $whatsNumber }}" target="_blank" aria-label="WhatsApp"><i class="fab fa-whatsapp"></i></a>
						@endif
						@if(!empty($empresa->website))
							<a href="{{ $empresa->website }}" target="_blank" aria-label="Website"><i class="fas fa-globe"></i></a>
						@endif
					</div>
				</div>
				...
				<div class="col-lg-3 col-md-6 mb-4">
					...
					<ul class="list-unstyled">
						...
						<li>
							...
							<a href="tel:+57{{ preg_replace('/[^0-9]/', '', $empresa->movil) }}" class="footer-link">{{ $empresa->movil }}</a>
						</li>
						@endif
						@if($hasEmail)
						<li>
	...
	<div class="modal fade gallery-modal" id="galleryModal" tabindex="-1" aria-hidden="true">
		<div class="modal-dialog modal-dialog-centered">
			<div class="modal-content">
				<div class="modal-header">
					<h5 class="modal-title" id="galleryModalTitle">Galería</h5>
	...
	<script>
		...
		function showGalleryImage(index) {
			...
			galleryModalTitle.textContent = image.description || 'Imagen';
		...
		function showPreviousImage() { currentImageIndex = currentImageIndex > 0 ? currentImageIndex - 1 : galleryImages.length - 1; showGalleryImage(currentImageIndex); }
		function showNextImage() { currentImageIndex = currentImageIndex < galleryImages.length - 1 ? currentImageIndex + 1 : 0; showGalleryImage(currentImageIndex); }
		...
		if (galleryModalImage) galleryModalImage.addEventListener('dragstart', function(e) { e.preventDefault(); });
	</script>
</body>
</html>
